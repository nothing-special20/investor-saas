...
```bash
mkvirtualenv investor_saas -p python3.9
...
Create a database named `investor_saas`.

```
createdb investor_saas
```
...
```bash
celery -A investor_saas worker -l INFO
```

Or with celery beat (for scheduled tasks):

```bash
celery -A investor_saas worker -l INFO -B
```
